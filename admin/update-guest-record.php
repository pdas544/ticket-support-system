...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $db->prepare("UPDATE guest_tickets SET agent_id = ?, status = ?, updated_at=? WHERE id = ?");
    $stmt->bind_param("issi", $assignedTo, $updatedStatus, $updatedAt,$ticketId);
    $result = $stmt->execute();

    if ($result) {
        header("Location: view-guest-tickets.php?update=true");
        exit();
    } else {
        header("Location: view-guest-tickets.php?update=false");
        exit();
    }
}
...
if (isset($_GET['id'])) {
    $ticketId = $_GET['id'];
    $stmt = $db->prepare("SELECT t.*, u.name AS user_name, a.name AS agent_name FROM guest_tickets t LEFT JOIN users u ON t.user_id = u.id LEFT JOIN agents a ON t.agent_id = a.id WHERE t.id = ?");
...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light px-3">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ms-auto">
                ...
                <li class="nav-item">
                    <a class="nav-link" href="view-guest-tickets.php">View Tickets</a>
                </li>
                <li class="nav-item">
    ...
    <div class="container w-25">
        ...
        <form id="updateRecordForm" method="POST" action="update-guest-record.php">
